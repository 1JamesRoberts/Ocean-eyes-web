...
describe('historyFilter', () => {
  ...
  it('includes a record inside the range', () => {
    const record = { timestamp: '2026-06-18T12:00:00' };
    expect(recordInRange(record, range)).toBe(true);
  });

  it('excludes a record before the range', () => {
    const record = { timestamp: '2026-06-18T08:00:00' };
    expect(recordInRange(record, range)).toBe(false);
  });

  it('excludes a record after the range', () => {
    const record = { timestamp: '2026-06-18T18:00:00' };
    expect(recordInRange(record, range)).toBe(false);
  });

  it('includes boundary records', () => {
    expect(recordInRange({ timestamp: '2026-06-18T09:00:00' }, range)).toBe(true);
    expect(recordInRange({ timestamp: '2026-06-18T17:00:00' }, range)).toBe(true);
  });
});
