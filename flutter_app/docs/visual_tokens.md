...
The live React implementation in `../../src/index.css` is the source of truth.
Flutter values are centralized in `lib/core/theme/oceaneyes_tokens.dart`; screen
widgets should not introduce duplicate raw palette or shell geometry values.

## Reference geometry

| Role | Flutter logical pixels | Web source |
|---|---:|---|
| Reference canvas | 393 × 852 | `--mobile-frame-width`; migration target |
| Hero action space | 393 × 221 | `--mobile-hero-height` |
| Hero fade extension | 56 | `--mobile-hero-blend-extension` |
| Rounded viewport top | 213 | main top 16 + CSS viewport offset 197 |
| Content gutter | 16 | `--mobile-content-gutter` |
| Card radius | 28 | `--glass-radius-card` |
| Inline radius | 12 | `--glass-radius-inline` |
| Navigation | x 16, h 64, bottom ≥ 12 | `.pill-nav` |
| Navigation radius | 32 | `.pill-nav` |
| Minimum effective target | 48 | Flutter accessibility adaptation |

Flutter logical pixels intentionally map 1:1 to CSS pixels at the reference
size. Safe-area values are added only outside the locked authored dimensions.
...
app canvas. The brighter CSS tokens remain unchanged for decorative fills,
...
CSS inset highlights with a restrained directional gradient and an outer
shadow. This is a reviewed platform rendering exception, not a new token.
...
Base tracking is `-0.15px`, the Flutter equivalent of the web `-0.01em` at
15px. Hero eyebrows use 12px/600 with 1.32px tracking.
